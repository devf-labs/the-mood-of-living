...
/**
 * Makes a custom Widget for displaying Recent Twitter Updates available with Razzo
 *
 * @package WordPress
 * @subpackage Razzo
 * @since Razzo 1.0
...
class twitter extends WP_Widget {
	...
	function twitter() {
		...
		$this->WP_Widget( 'widget_recent_twitter_updates', __( 'Twitter', 'mega' ), $widget_ops, $control_ops );
	}

	function widget( $args, $instance ) {
 		extract($args);

		$title = apply_filters('widget_title', empty($instance['title']) ? __( 'Twitter', 'mega' ) : $instance['title'], $instance, $this->id_base);
